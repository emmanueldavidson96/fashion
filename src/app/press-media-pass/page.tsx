...
import { base_afwn_press_media } from '../../../api/base';
...
export default function page() {
    ...
    const handle_submit = (event:React.FormEvent<HTMLFormElement>) => {
        ...
        base_afwn_press_media("PressMedia").create({
            Name, OfficialEmailAddress, CompanyName, WebsiteURL, JobTitle
  ...
  return (
    <div className=''>
        ...
        <div className='h-[400px] w-full bg-black flex gap-5 flex-col items-center justify-center'>
            <h2 className='text-2xl text-center w-[90%] mx-auto lg:text-4xl font-extrabold uppercase text-white mt-8'>Press and Media Pass</h2>
            ...
            <div className='w-[92%] mx-auto flex max-lg:flex-col max-lg:gap-8 justify-between'>
                ...
                <div className='lg:w-[48%] w-full  flex flex-col gap-4'>
                    <form className='w-full mx-auto flex flex-col gap-6' onSubmit={handle_submit}>
                        <label className='text-2xl lg:text-4xl text-black font-bold uppercase'>Press and Media Pass Form</label>
                        <input type="text" name="" id="" 
                            onChange={(e) => setName(e.target.value)}
                            className='w-full px-6 py-3 border-b text-black border-green-500' placeholder='Name' />
                        <input type="email" name='' id='' 
                            onChange={(e) => setOfficialEmailAddress(e.target.value)}
                            className='w-full px-6 py-3 border-b text-black border-green-500' placeholder='Official Email Address'/>
                        <input type="text" name='' id='' 
                            onChange={(e) => setCompanyName(e.target.value)}
                            className='w-full px-6 py-3 text-black border-b border-green-500' placeholder='Company Name'/>
                        <input type="text" name='' id='' 
                            onChange={(e) => setWebsiteURL(e.target.value)}
                            className='w-full px-6 py-3 border-b text-black border-green-500' placeholder='Website / URL'/>
                        <input type="text" name='' id='' 
                            onChange={(e) => setJobTitle(e.target.value)}
                            className='w-full px-6 py-3 text-black border-b border-green-500' placeholder='Job Title'/>                       
